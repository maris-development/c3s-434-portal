...
#BASE_URL = 'https://cds.climate.copernicus.eu/apps/136/c3s_434_meanradianttemperature_detail_web'
...
indicator           = 'mean_radiant_temperature'
...
#        loopyears = [str(yr) for yr in range(syr, syr+retrieve_year_spacing)]
    ...
    # get monthly average of selected month and region and all years
    #################################################################################################
...
def plot_grid(layer, layer_non, dummy, var, clickkwargs):
    ...
    livemap_data = {
        'data': layer,
        'label': var,
        'label_template': '%{name} %{value:.2f}°C',
        'type':'layer',
    ...
    livemap_data_non = {
        'data': layer_non,
        'label': var,
        'label_template': '%{NAME_EN} %{value:.2f}°C',
        'type':'layer',
        'zoom_to_selected': False,
        'checked':True,
        'click_kwargs': clickkwargs,
    ...
    livemap_data_kosovo = {
        'data': kosovo,
        'label': var,
        'label_template': 'Kosovo (under UNSCR 1244/99) %{value:.2f}°C',
        'type':'layer',
        'zoom_to_selected': False,
        'checked':True,
        'click_kwargs': clickkwargs,
...
@ct.application(title='Mean radiant temperature', layout=layout)
...
@ct.input.dropdown('month', label = 'Month of interest',
                   values=MONTH_NAMES,default=MONTH_NAMES[1],
                   description = 'Choose a month',
                   help = 'Display data for the chosen month and year of the dataset',
                  )
...
def application(year, month, nutslevel):
    ...
    dummydata = ct.cube.index_select(data_year_cel-999)

    livemap = plot_grid(layer=data_year_nuts1, layer_non=data_year_nuts_non1, dummy=dummydata, var=indicator, clickkwargs=child_kwargs)
