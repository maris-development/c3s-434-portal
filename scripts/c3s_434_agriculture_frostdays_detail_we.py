# c3s_434_agriculture_frostdays_detail_web\
...
europe_bb = [-25, 49, 34, 71]  # min_lon, max_lon, min_lat, max_lat
...
fig1text = '''
## Change in number of frost days in {} {}
Showing the range of changes in number of frost days across an ensemble of {} climate models.
Hover over the graph to see the values.
'''

fig2text = '''
## Historical climate
Showing average number of frost days per month in the period {}-{}
...
filter_help = {
    'speriod': "Choose a 30-year period average",
...
magics_styles = {"Annual": "red_yellow_-160_0",
                 "Monthly": "red_yellow_-31_4",
                 }
...
def count_days_threshold(data, threshold=0, how="above"):
    ...
    if how == "above":
        ...
        raise Exception(
            'Unknown keyword in count_days_threshold(). The value of how was: {}'.format(how))
...
def plot_shapes(darray, darray2, var='unknown', exp='', mon=1, dummy=None):
    ...
    dummy = ct.cdm.update_attributes(
        dummy, {'cds_magics_style_name': mystyle, })
...
def plot_climatology(mean, sd, pc0, pc1):
    ...
    climfig = ct.chart.line(pc0, fig=climfig,
                            scatter_kwargs={
                                'name': '5th percentile', 'mode': 'lines'},
    ...
    climfig = ct.chart.line(pc1, fig=climfig,
                            scatter_kwargs={
                                'name': '95th percentile', 'mode': 'lines'},
                            line={'dash': 'dot', 'width': 3},
                            layout_kwargs=layout_kwargs)
...
@ct.application(title='Number of frost days',
                layout=layout)
...
def workflow(speriod, smon, rcp, nuts_level):
    ...
    for y in range(period_hist[0], period_hist[1]+1):
        ...
        for m in range(1, 13):
            ...
            datarray_m_tot = count_days_threshold(
                datarray_m_sub, threshold=273.15, how="below")
    ...
    datarray_clim = ct.climate.climatology_mean(
        data_nuts_avg, start=y0, stop=y1, frequency='month')
    datarray_clim_sd = ct.climate.climatology_std(
        data_nuts_avg, start=y0, stop=y1, frequency='month')
    datarray_clim_pc = ct.climate.climatology_perc(data_nuts_avg, start=y0, stop=y1,
                                                   percentiles=[5., 95.], frequency='month')

    datarray_non_nuts_clim = ct.climate.climatology_mean(
        data_non_nuts_avg, start=y0, stop=y1, frequency='month')
    datarray_non_nuts_clim_sd = ct.climate.climatology_std(
        data_non_nuts_avg, start=y0, stop=y1, frequency='month')
    ...
    for gcm in allgcms:  # LOOP FROM HERE
        ...
        datarray_gcm_mon = ct.cube.resample(
            datarray_gcm_sub, how='sum', freq='month')
        ...
        # calculate monthly totals
        datarray_hist_mon = ct.cube.resample(
            datarray_hist_sub, how='sum', freq='month')
    ...
    anom_nuts_avg, anom_non_nuts_avg = avg_nuts(
        anom_ens_avg, nutslevel=nuts_level)
    anom_nuts_max, anom_non_nuts_max = avg_nuts(
        anom_ens_max, nutslevel=nuts_level)
    anom_nuts_min, anom_non_nuts_min = avg_nuts(
        anom_ens_min, nutslevel=nuts_level)

    # select month of interest
    if mymonth == 0:
        anom_nuts_avg_mon = ct.cube.resample(
            anom_nuts_avg, how='sum', freq='year')
        anom_nuts_max_mon = ct.cube.resample(
            anom_nuts_max, how='sum', freq='year')
        anom_nuts_min_mon = ct.cube.resample(
            anom_nuts_min, how='sum', freq='year')

        anom_non_nuts_avg_mon = ct.cube.resample(
            anom_non_nuts_avg, how='sum', freq='year')
        anom_non_nuts_max_mon = ct.cube.resample(
            anom_non_nuts_max, how='sum', freq='year')
        anom_non_nuts_min_mon = ct.cube.resample(
            anom_non_nuts_min, how='sum', freq='year')
    else:
    ...
    dummydata = ct.cube.index_select(anom_ens_avg-999, time=mymonth)
